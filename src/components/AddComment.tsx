...
export default function AddComment({bookID}:{bookID:string}){
  ...
  return(
    <>
    <p className="w-52 text-center text-2xl font-bold bg-zinc-900
      bg-opacity-80  my-2 px-2 py-2 border rounded-lg mt-3">New Comment: 
    </p>
    <textarea rows={4}  name={comment} className="bg-zinc-900 bg-opacity-80 
      border rounded-md my-1 px-3 py-2 focus:bg-zinc-900" value={comment || ""}
      onChange={(e)=>setComment(e.target.value)}/>
    <button className="mx-auto w-48 text-xl bg-blue-600 border-black shadow-lg
